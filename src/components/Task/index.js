...
const Task = (item, deleteTasks, userItem, deleteTask, updateTask) => { /// نستقبل البروبز
    ...
    <div>
      {state.sign.role === "admin" ? ( // في البيانات اللي جابتها اليوز سلكتور إذا نوع اليوزر = ادمن ؟ 
        ...
        <li key={userItem._id}>
          ...
          <button onClick={() => deleteTask(userItem._id)}>delete</button> {/*نرسل الايدي تبع التاسك اللي ابغى اعمل لها ديليت */}
        </li>
      )}
    </div>
  );
